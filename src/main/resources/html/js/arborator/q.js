...
call.print("q started!");


conlls = {	10: 	{"id": 0, "t":1, "lemma": 2, "cat": 3, "gov":6, "func":7}, 
...
settings = function() 
	{
		...
		attris = {"t":		{"font": '18px Arial', "text-anchor":'start',"fill": '#000',"cursor":'move',"stroke-width": '0'}, 
			...
			"source":	{"fill": '#654b6d'},
			"target":	{"fill": '#dd137b',"cursor": 'url("{connect}"),pointer', "font-style":'italic'}, 
	...
	for (var i in words)
		{
			...
			{	
				...
				else if (el == 10) 	ar = [n.index,n.features.t,n.features.lemma,n.features.cat,"_","_","_","_","_","_"]
...
// 					console.log("!!!",func);
					...
					else if (el == 10) 	ar = [n.index,t,n.features.lemma,n.features.cat,"_","_",j,func,"_","_"]
...
// 			$('#source').attr("value",header+$('#holder'+nr).html());
	...
	if(v=="xxxsvg")  // TODO: no need to go via the server to show svg content
	{
		var x=window.open();
		x.document.open();
		x.document.write(header+$('#holder'+nr+' >svg').clone().wrap('<p>'));
		x.document.close();	
	}
	else $('#ex').submit();
...
// $( document ).ready(function() {
	call.print("anonym function started!!");
// 	console.log( "ready!" );	
	settings();
	call.print("settings invoked!");
	readConll();
	call.print("readconll invoked!");
	setupStyleDialog() 
	call.print("setupstyledialog invoked!");
	drawTrees();
	call.print("drawtrees invoked!");

	$("#sentences").val(sentencesText)
	$('#conll').focus(function() {
		call.print("conll focused!");
		nokeys=true;
	});
	$('#conll').blur(function() {
		call.print("conll blured!");
